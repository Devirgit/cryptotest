...
class ViewFormat {
  ...
  static String formatingDateWeek(
    DateTime date,
  ) {
    final now = DateTime.now();
    if (now.year == date.year && now.month == date.month) {
      if (now.day - date.day <= 3) {
        return UItext.thisWeek[now.day - date.day];
      }
    }
    return "${UItext.weekDay(date.weekday)} , "
        "${UItext.datePickerShortMonth(date.month)} ${date.day}, ${date.year}";
  }
}
