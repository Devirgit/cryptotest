...
class InfiniteList<T> extends StatefulWidget {
  ...
  final ListWidgetBuilder<T> separatorBuilder;
...
class _InfiniteListState<T> extends State<InfiniteList<T>> {
  ...
  Widget _loadedData(List<T> items, {Widget? loading}) {
    ...
    return ListView.separated(
        itemCount: loading == null ? count + 1 : count + 2,
        controller: _controller,
        itemBuilder: (BuildContext context, int index) {
          if (index == 0) {
            return widget.separatorBuilder(context, index, items);
          }
          ...
          return widget.itemBuilder(context, index - 1, items);
        },
        separatorBuilder: (context, index) {
          if (index != 0 && index <= items.length) {
            return widget.separatorBuilder(context, index, items);
          }
          return widget.separate ??
              const SizedBox(
                width: 1,
              );
        });
